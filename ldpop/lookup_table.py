...
class LookupTable(pandas.DataFrame):
    ...
    Printing
    --------
    str(lookup_table) returns a string in the format expected by ldhat (or ldhelmet, if the rhos are not a uniformly spaced grid starting at 0)
    ...
    lookup_table.pop_sizes = [size0,size1,...,sizeD] = coalescent scaled population sizes
         size0 = present size, sizeD = ancient size
    lookup_table.times = [t1,...,tD] = the times of size changes, going backwards from present
         must be increasing positive reals
    lookup_table.exact = boolean
         if False, use finite moran model, a reasonable approximation that is much faster than the exact formula. Accuracy of the approximation can be improved by taking n larger than needed, and subsampling (e.g. using ldhat/lkgen.c). As n->infty, the finite moran model -> 'exact' diffusion.
