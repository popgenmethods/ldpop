...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Print a table of (approximate) likelihoods through time, '
                    'use in conjunction with bin/ImportanceSampler.jar')
    parser.add_argument('-n', type=int, metavar='N', required=True,
                        help='sample size (table will contain configs '
                             'with 2N haplotypes)')
    parser.add_argument('-th', type=float, metavar='theta', required=True,
                        help='twice the mutation rate')
    parser.add_argument('-rh', type=str, metavar='rho', required=True,
                        help='twice the recomb rate.  Alternatively, one can '
                             'input num_rho,max_rho to make a grid with '
                             'num_rh uniformly spaced points from 0 to '
    ...
    parser.add_argument('--grdpts', default=5, metavar='D', type=int,
                        help='Number of points per epoch to discretize time. '
                             'More points will results in a longer run time, '
                             'but more efficient importance sampling.')
    ...
    print(ISProposal(args.n, args.th, rhos, popSizes,
                     times, args.grdpts, args.cores))
