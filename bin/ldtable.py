...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Print a lookup table, as expected by ldhat or ldhelmet.")
    parser.add_argument("-n", type=int, metavar="N", required=True, help="sample size")
    parser.add_argument("-th", type=float, required=True, metavar="theta", help="twice the mutation rate")
    parser.add_argument("-rh", type=str, required=True, metavar="num_rh,max_rh", help="grid of rhos (twice the recomb rate). The grid has num_rh uniformly spaced points from 0 to max_rh, inclusive. (((Alternatively, to create a non-uniform grid, use '-rh r0,step0,r1,step1,r2,...rK'. This creates a grid {r0,r0+step0,r0+2*step0,...,r1,r1+step1,...,rK} similar to ldhelmet. Note that non-uniform grid is incompatible with vanilla ldhat.)))")
